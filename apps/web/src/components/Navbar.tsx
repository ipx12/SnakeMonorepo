...
export function Navbar() {
  ...
  return (
    <header className="sticky top-0 z-50 w-full max-w-full border-b border-border/60 bg-background/80 backdrop-blur-xl">
      <div className="max-w-7xl mx-auto px-3 sm:px-6 lg:px-8 h-16 flex items-center justify-between gap-2">
        {/* Brand logo */}
        <Link href="/" className="flex items-center gap-2 group transition-transform active:scale-95 shrink-0">
          <div className="size-8 sm:size-9 rounded-xl bg-linear-to-br from-emerald-400 via-teal-500 to-indigo-600 flex items-center justify-center shadow-lg shadow-emerald-500/20 group-hover:shadow-emerald-500/30 transition-all">
            <Sparkles className="size-4 sm:size-5 text-white animate-pulse" />
          </div>
          <div className="flex flex-col">
            <span className="font-extrabold text-base sm:text-lg tracking-tight bg-linear-to-r from-emerald-400 via-teal-300 to-indigo-300 bg-clip-text text-transparent">
              Watermelon
            </span>
            <span className="hidden sm:inline text-[10px] text-muted-foreground -mt-1 font-medium tracking-wider uppercase">
              Monorepo Suite
            </span>
          </div>
        </Link>
        ...
        <nav className="flex items-center gap-1.5 sm:gap-4 shrink-0">
          ...
          {!isAuthLoading && user ? (
            <div className="flex items-center gap-2 sm:gap-3 pl-1.5 sm:pl-2 border-l border-border/80">
              <div className="hidden md:flex items-center gap-2.5 px-3 py-1 rounded-full bg-secondary/40 border border-border/50">
              ...
              <Button
                ...
                size="sm"
                onClick={() => logout()}
                className="h-8 px-2 sm:px-3 text-xs font-medium text-muted-foreground hover:text-destructive hover:bg-destructive/10 transition-colors gap-1.5 cursor-pointer"
                title="Sign Out"
              >
